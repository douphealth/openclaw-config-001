...
**Do NOT use for:** simple single-threaded tasks (do them directly), tasks needing tight interactive back-and-forth, decomposition that adds more overhead than value.
...
|-------------|-------------|-----------|
| Vague, overlapping worker tasks | Workers produce inconsistent outputs | Define clear, non-overlapping deliverables |
| Skipping verification | Errors slip through | Always assign verifier for non-trivial work |
| Main thread doing worker tasks | Loses orchestration perspective | Keep director focused on coordination |
| Decomposing a 5-minute task | More overhead than value | Only decompose when time savings are real |
| No quality gates defined | "Done" becomes subjective | Define gates before launching |
| Aggressive retry on failure | Thrashing, wasted tokens | After 2 failures, escalate or change approach |
| Workers reporting optimism as fact | False completion claims | Require concrete evidence in outputs |
