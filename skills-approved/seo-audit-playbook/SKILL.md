...
**Do NOT use for:** writing/editing content (→ `editorial-post-enhancement`), implementing schema (→ `schema-ops`), setting up tracking (→ `tracking-measurement`), content strategy (→ `content-strategy-planning`).
...
### Audit Speed Protocol
- Crawlability check: 2-3 minutes (parallel robots.txt + sitemap + GSC)
- Technical check: 5-10 minutes (parallel page checks)
- Cannibalization: 3-5 minutes (REST API bulk title/meta comparison)
- On-page: 2-3 minutes per page (batch where possible)
- Full audit target: 20-30 minutes (not 60+)
