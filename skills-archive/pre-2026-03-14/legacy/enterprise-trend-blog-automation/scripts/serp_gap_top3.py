...
if __name__=='__main__':
    ...
    ap.add_argument('--top3-json',required=True)
    ap.add_argument('--draft-path',required=True)
    ap.add_argument('--limit',type=int,default=20)
    a=ap.parse_args()
    comp=collections.Counter()
    for t in json.loads(a.top3_json):
        comp.update(terms(t))
    draft=open(a.draft_path,encoding='utf-8',errors='ignore').read()
    dset=set(terms(draft))
    missing=[{'term':k,'score':v} for k,v in comp.most_common(800) if k not in dset]
    print(json.dumps({'top_missing':missing[:a.limit],'count':min(a.limit,len(missing))},indent=2))
