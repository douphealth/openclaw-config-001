...
## Do NOT Use For
- On-page content editing (use `editorial-post-enhancement`)
- WordPress operations (use `wp-rest-api-mastery` or `wordpress-growth-ops`)
- Site-wide SEO audit with no search-console/webmaster focus (use `seo-audit-playbook`)
- Competitor-only intelligence work (use `seo-competitor-analysis`)
...
## Competitor-Validated Query Expansion Rule
When moving from a keyword opportunity to a page recommendation:
1. identify the true keyword-targeting competitors from the live SERP
2. keep only pages whose titles and URLs clearly target that keyword
3. extract overlapping subtopics/secondary terms from those pages
4. use the overlap set as the minimum on-page topical blueprint
5. if backlink data is available, prioritize domains linking to multiple such competitor pages
...
### API Performance
- Apply `skills/api-efficiency-protocol.md` for GSC/Bing data pulls
...
- Pull the smallest dimension set needed to make the next decision
