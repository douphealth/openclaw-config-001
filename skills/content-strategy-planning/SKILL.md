...
## Do NOT Use For
- Writing or upgrading individual articles (use `editorial-post-enhancement`)
- SEO auditing and technical diagnosis (use `seo-audit-playbook`)
- Conversion copy for marketing pages (use `conversion-copywriting`)
- Offer definition or funnel architecture (use `offer-positioning` or `service-funnel-architecture`)
...
### Speed Multipliers
- Apply `skills/api-efficiency-protocol.md` when collecting CMS, analytics, or SERP-supporting API data
- Batch page analysis (10-20 per call)
- Pre-crawl data before live crawl
- Template-based reports
- Parallel checks where possible
- Pull only decision-relevant fields from CMS/content APIs before planning
